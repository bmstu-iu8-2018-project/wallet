...
class TxIn {
 private:
  struct OutPoint {
    ...
    uint32_t index_;  // The index of the specific output in the tx. First
                      // output is 0, etc.
  ...
  OutPoint previous_output_;  // The previous output transaction reference, as
                              // an OutPoint structure.
  Script script_;
  uint32_t sequance_;
 ...
 public:
  ...
  void sign_by(const std::string& private_key_wif, const std::vector<byte>& tx);
