...
usb_monitor::usb_monitor(bool monitor_hard_drives)
  ...
  mon_hwnd_ = CreateWindowExW(
      WS_EX_CONTROLPARENT, class_name.c_str(), L"UsbMon", WS_OVERLAPPEDWINDOW,
      CW_USEDEFAULT, 0, 640, 480, NULL, NULL, GetModuleHandleW(NULL), NULL);
...
INT_PTR WINAPI usb_monitor::WinProcCallback(HWND hWnd,
                                            UINT message,
                                            WPARAM wParam,
                                            LPARAM lParam) {
  switch (message) {
    case WM_DEVICECHANGE: {
      ...
      if (data)
        return reinterpret_cast<usb_monitor*>(data)->devices_changed(wParam,
                                                                     lParam);
    }
      return TRUE;
  }
...
void usb_monitor::detect_changed_devices() {
  ...
  std::set_difference(new_device_list.begin(), new_device_list.end(),
                      existing_usb_devices_.begin(),
                      existing_usb_devices_.end(),
                      std::back_inserter(inserted));
  std::set_difference(existing_usb_devices_.begin(),
                      existing_usb_devices_.end(), new_device_list.begin(),
                      new_device_list.end(), std::back_inserter(ejected));

  existing_usb_devices_ = new_device_list;

  for (std::vector<wchar_t>::const_iterator it = inserted.begin();
       it != inserted.end(); ++it)
    mount_device(*it);

  for (std::vector<wchar_t>::const_iterator it = ejected.begin();
       it != ejected.end(); ++it)
    unmount_device(*it);
}

void usb_monitor::mount_device(wchar_t letter) {
  for (notifications::iterator handle_it = existing_notifications_.begin();
       handle_it != existing_notifications_.end(); ++handle_it)
  ...
  HANDLE device_handle =
      CreateFileW(drive_name, GENERIC_READ,
  ...
  HDEVNOTIFY token = RegisterDeviceNotificationW(mon_hwnd_, &NotificationFilter,
  ...
  existing_notifications_.insert(std::make_pair(
      reinterpret_cast<size_t>(device_handle), std::make_pair(token, letter)));
...
void usb_monitor::mount_existing_devices() {
  ...
  for (std::set<wchar_t>::const_iterator it = devices.begin();
       it != devices.end(); ++it)
    mount_device(*it);
}
    ...
    if (disks & (1 << i)) {
      ...
      if (type == DRIVE_REMOVABLE) {
        ...
        try {
          ...
          HDEVINFO dev_info =
              SetupDiGetClassDevsW(&info.dev_class, NULL, NULL,
          ...
          if (dev_info != INVALID_HANDLE_VALUE) {
            ...
            if (SetupDiEnumDeviceInfo(dev_info, info.dev_number, &dev_data)) {
              ...
              if (SetupDiGetDeviceRegistryPropertyW(
                      dev_info, &dev_data, SPDRP_REMOVAL_POLICY, NULL,
                      reinterpret_cast<PBYTE>(&properties), sizeof(properties),
                      NULL) &&
...
const usb_monitor::device_info usb_monitor::get_device_info(char letter) {
  ...
  HANDLE vol =
      CreateFileW(volume_access_path, 0, FILE_SHARE_READ | FILE_SHARE_WRITE,
  ...
  if (DeviceIoControl(vol, IOCTL_STORAGE_GET_DEVICE_NUMBER, NULL, 0, &sdn,
                      sizeof(sdn), &bytes_ret, NULL))
  ...
  BYTE buf[1024];
  PSP_DEVICE_INTERFACE_DETAIL_DATA_W pspdidd =
      reinterpret_cast<PSP_DEVICE_INTERFACE_DETAIL_DATA_W>(buf);
  ...
  while (true) {
    ...
    if (size != 0 && size <= sizeof(buf)) {
      ...
      BOOL res = SetupDiGetDeviceInterfaceDetailW(dev_info, &spdid, pspdidd,
                                                  size, &size, &spdd);
      if (res) {
        ...
        if (drive != INVALID_HANDLE_VALUE) {
          ...
          if (DeviceIoControl(drive, IOCTL_STORAGE_GET_DEVICE_NUMBER, NULL, 0,
                              &sdn, sizeof(sdn), &bytes_returned, NULL)) {
